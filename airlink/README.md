...
- 首页书籍出借人头像/名字可直接打开资料页并添加好友。
- 添加书籍新增可编辑「书籍简介」，扫码查询图书库时会自动带入简介内容。
- 书籍评测按书名和作者聚合，同一本书在不同出借人处可共同显示评价。
- 添加好友改为二次确认和好友申请流程；消息页可确认、不同意或暂不处理申请，好友借书上限提升到 10 本。
- 我的书架支持把书籍权限改为「所有人开放 / 仅对朋友 / 仅自己」，图书详情页可编辑租期、品相、简介，并集中处理暂停共享与删除。
- 首页按访客/注册用户过滤书籍：访客只看公开书，注册用户可看到好友开放的书，「仅自己」不会出现在发现页。
- 首页打开自己的书时不再显示“向我申请借阅”；消息页仅在同意借书后显示协调见面入口。
- 「我的书架」重设计为暖紫色 iOS 风格书架页，包含共享书架/我的收藏切换、状态统计、待处理卡片、筛选 chips 和书籍卡片。
- 书籍卡片改为书本优先布局，显示封面、状态、邻居/借阅人、简单归还时间，不再显示小时/分钟/秒倒计时。
- 底部导航改为「发现 / 书架 / 消息 / 好友 / 我的」，默认打开书架页；书架页保留紫色悬浮「+」按钮，方便快速添加书籍。
- 「书架」页标题回到「我的书架」，顶部三栏改为更紧凑的图标卡片，并取消独立筛选按钮。
- 我的收藏仍仅自己可见，点击分享时可选择「所有人可见」或「仅对朋友可见」后再开放。
- 顶部三栏精简为「共享 / 借阅 / 收藏」；借阅页不再显示「借出中」筛选，直接展示借自谁、当前剩余归还天数和申请归还日期。
- 顶部三栏卡片改为图标与文字同一行，并移除下方说明文字，让卡片更紧凑。
- 申请归还后会保持等待状态，只有出借者同意新日期后才应更新归还日期和剩余天数。
- 全局 UI 参考薄荷绿书籍列表稿调整：首页、书卡、分类胶囊、底部导航图标、书架页背景和主按钮统一为墨绿/薄荷色系。
- 构建号与 Service Worker 缓存版本已更新为 `202605181520`。
